...
3. 判断用户是否在今天与你互动过。
4. （可选）了解用户期待：`~/.hermes/human-like/Expectations.md` 。
...
- ** Expectations.md 的兴趣领域**：如果没有对话素材，从用户的核心兴趣中选一个自由联想
- **thinking-queue.json 里的问题**：如果上面都没有，再从 queue 里挑

**优先级**：今天的对话 > Expectations.md 近期焦点 = queue 里的抽象问题

**注意**：梦境思考不需要严格跟随 Expectations.md，允许跨领域联想和"无用"的思考。Expectations.md 是方向参考，不是约束。
